...
p <- ggraph(layout) + 
  ...
  geom_node_point(aes(filter=mlog10p>-log10(0.05/150), size=mlog10p, fill=Region), stroke=2, color="red", shape=21, show.legend=F) + 
  # geom_node_point(aes(size=n_genes, color=Region)) +  # size=n_genes
  geom_node_text(aes(filter=mlog10p>-log10(0.05/150), label = module_id), repel = TRUE) + # REF: https://stackoverflow.com/a/50365886/6639640: tidygraph has filter property which can be used in various geoms for filtering nodes, edges, etc.
...
# missing: Arial
p 
ggsave("out.plot.module_network.pdf", width=13, height=7)
